...
PAPI_SET_NAMESPACE_N GetApiSetNamespace()
{
	ULONG	ReturnLength;
	PROCESS_BASIC_INFORMATION ProcessInformation;
	PAPI_SET_NAMESPACE_N apiSetMap = NULL;
	...
	apiSetMap =  reinterpret_cast<PAPI_SET_NAMESPACE_N>(peb->ApiSetMap);
...
{
    static ApiSetSchema^ ParseApiSetSchema(API_SET_NAMESPACE_N const * apiSetMap);
...
ApiSetSchema^ PE::GetApiSetSchema()
{
    ...
    for (auto n = 0u; n < mappedImage.NumberOfSections; ++n)
    {
        ...
        if (strncmp(".apiset", reinterpret_cast<char const*>(section.Name), IMAGE_SIZEOF_SHORT_NAME) == 0)
            return ApiSetSchemaImpl::ParseApiSetSchema(reinterpret_cast<PAPI_SET_NAMESPACE_N>(PTR_ADD_OFFSET(mappedImage.ViewBase, section.PointerToRawData)));
    }
    return gcnew EmptyApiSetSchema();
}

ApiSetSchema^ ApiSetSchemaImpl::ParseApiSetSchema(API_SET_NAMESPACE_N const * const apiSetMap)
{
